...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    if (elem.retweets > mostChild.retweets) {
      elem
    } else {
      mostChild
    }
  ...
  def descendingByRetweet: TweetList = {
    val remainSet = remove(mostRetweeted)
    if (remainSet.isEmpty) new Cons(mostRetweeted, Nil)
    else new Cons(mostRetweeted, remainSet.descendingByRetweet)
  }
